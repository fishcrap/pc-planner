...
class Loss(nn.Module):
    def __init__(self, weights=[3e3, 1e2, 1e2, 5e1, 1e2, 1e2, 0], loss_type='siren', div_decay='none', div_type='dir_l1'):
        super().__init__()
        ...
        self._loss_names = ["loss", 'sdf_term', 'inter_term', 'latent_reg_term',
                            'eikonal_term', 'normals_loss', 'div_loss',
                            'dist_loss'
        ]
    ...
    def forward(self, output_pred, mnfld_points, nonmnfld_points, nonmnfld_dists, mnfld_n_gt=None):
        ...
        # dists
        dist_loss = torch.abs(non_manifold_pred.squeeze() - nonmnfld_dists.squeeze()).mean()
        ...
        if self.loss_type == 'siren': # SIREN loss
            loss = self.weights[0]*sdf_term + self.weights[1] * inter_term + \
                   self.weights[2]*normal_term + self.weights[3]*eikonal_term + self.weights[5] * dist_loss
        elif self.loss_type == 'siren_wo_n': # SIREN loss without normal constraint
            self.weights[2] = 0
            loss = self.weights[0]*sdf_term + self.weights[1] * inter_term + self.weights[3]*eikonal_term + self.weights[5] * dist_loss
        elif self.loss_type == 'igr': # IGR loss
            self.weights[1] = 0
            loss = self.weights[0]*sdf_term + self.weights[2]*normal_term + self.weights[3]*eikonal_term + self.weights[5] * dist_loss
        elif self.loss_type == 'igr_wo_n': # IGR without normals loss
            self.weights[1] = 0
            self.weights[2] = 0
            loss = self.weights[0]*sdf_term + self.weights[3]*eikonal_term + self.weights[5] * dist_loss
        elif self.loss_type == 'siren_w_div': # SIREN loss with divergence term
            loss = self.weights[0]*sdf_term + self.weights[1] * inter_term + \
                   self.weights[2]*normal_term + self.weights[3]*eikonal_term + \
                   self.weights[4] * div_loss + self.weights[5] * dist_loss
        ...
        return {"loss": loss, 'sdf_term': self.weights[0]*sdf_term, 'inter_term': self.weights[1]*inter_term, 'latent_reg_term': self.weights[6]*latent_reg_term,
                'eikonal_term': self.weights[3]*eikonal_term, 'normals_loss': self.weights[2]*normal_term, 'div_loss': self.weights[4]*div_loss,
                'dist_loss': self.weights[5]*dist_loss
                }
